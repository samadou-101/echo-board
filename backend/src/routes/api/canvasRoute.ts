...
router.post("/save-canvas", async (req: Request, res: Response) => {
  try {
...
router.put("/update-canvas/:canvasId", async (req: Request, res: Response) => {
  try {
...
// Load canvas by ID
router.get("/load-canvas/:canvasId", async (req: Request, res: Response) => {
  try {
    const { canvasId } = req.params;
    const userId = req.headers["x-user-id"] as string;
...
router.get("/list-canvases", async (req: Request, res: Response) => {
  try {
    const userId = req.headers["x-user-id"] as string;
    ...
    const canvasQuery = db.collection("canvases").where("userId", "==", userId);
